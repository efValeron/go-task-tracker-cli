...
func ChangeStatus(id uuid.UUID, newStatus types.StatusNames) (error bool) {
	...
	newStatus = types.StatusNames(strings.ToUpper(string(newStatus)))

	switch newStatus {
	case types.TODO, types.IN_PROGRESS, types.DONE, "":
		utils.DebugLog(color.BlueString("Changing task status to: ")+color.CyanString("%v\n"), newStatus)
	default:
		fmt.Println(color.RedString("Invalid status!"))
		return
	}
	...
	for i, task := range tasksJSON.Tasks {
		if task.ID == id {
			tasksJSON.Tasks[i].Status = newStatus
	...
	fmt.Println(color.GreenString("Task status updated successfully!"))
	return
}
